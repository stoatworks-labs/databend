...
/**
    ...
    **It reallocates only when it has to.** Ensure() is called every frame and
    ...
    host's picture changes size or the stream's layout, padding or direction moves. A
    reallocation clears, which is safe here because nothing on the GPU
    outlives a frame: every stream buffer is rewritten from the start each frame (see
    `Databend.h`).
    ...
    **It owns its filtering.** Every buffer in this plugin is samples of the
    stream read texel for texel with `texelFetch` -- a texel is one PCM
    sample, and a filtered read between two is a read between two samples
    of the file -- so every buffer here is Nearest. Linear and Mipmapped are
    tinsel's and are unused.

    Copied from tinsel (by way of standards, clamp and slope), where it was
    written for a glow chain; the mechanism is unchanged.
*/
