...
test card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/dbtest] [--size WxH] [--jobs N] [--allow-no-gl]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The card has to have edges and change.** A delay-line effect on a flat
field returns the flat field; the card carries colour bars (hard edges in
every channel), a ramp, a block that cuts and a moving bar, so a delay, a
comb or a smear has somewhere to show. `dbtest --out` renders it for 40
frames, which also moves the LFOs' phase.

**Some controls need another to mean anything.** Overflow only shows where
the signal passes full scale, so it is swept with the echo at full feedback
and mix; the pitch controls only matter with Pitch On, and Pitch On with a
Ratio away from 1. CONTEXT says what else has to be true for each, and uses
nothing but the plugin's own parameters.
...
`dbtest --list` prints the count as the range, and the ends swept are the
first and last element; the four counts list their real ranges and are
swept at both ends.
...
CONTEXT = {
    "Overflow": {"Feedback": 1, "Echo Mix": 1},
    "Pitch On": {"Ratio": 0},
    "Ratio": {"Pitch On": 1},
    "Grain": {"Pitch On": 1, "Ratio": 0},
    "Pitch Mix": {"Pitch On": 1, "Ratio": 0},
}
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "dbtest"))
    ...
    scratch = tempfile.mkdtemp(prefix="dbsweep")

    # One render first: a runner with no GL at all cannot sweep anything, and
    # that has to read as a skip, loudly, not as twenty-five dead controls.
