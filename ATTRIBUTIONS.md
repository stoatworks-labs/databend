...
Databend is built on other people's work. This file lists what that work is, who
did it, and what it is doing here.

> **Provisional.** Across the fleet this file is generated from master lists in
> `stoatworks-backend` by `scripts/sync-attributions.py`. Databend is not
> registered there yet, so this copy is hand-written in the generated shape.
> Register it before release — and note that the script's `--only` flag
...
<https://github.com/stoatworks-labs/slope>, <https://github.com/stoatworks-labs/clamp>
Licence: MIT
...
The plugin ABI itself. An FFGL effect is defined by this SDK's headers — there is no other way to be loadable by Resolume Arena and Avenue.
...
Windows only, from vcpkg, statically linked. The SDK's headers pull it in for the OpenGL function pointers; macOS uses the system OpenGL framework instead.
...
Arrives inside the FFGL submodule, under the SDK's CustomThumbnail sample. Part of the upstream SDK tree rather than something this plugin calls directly — listed because it is present in the checkout.
