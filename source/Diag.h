...
/**
    ...
    **A buffer that could not be allocated.** This plugin holds two stream
    buffers, one sample of the stream per texel, L samples wide and lines
    (times three for a planar layout) tall, both R32F, and the failure mode when the driver says no is a
    black frame with nothing to explain it. What goes in the log is the size
    asked for: at 4K interleaved the pair comes to about 200 MB.
*/
namespace databendfx::diag
{
