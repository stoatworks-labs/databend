...
#                                host reads SW Databend / DB01 / effect
#                   --window     the phaser's restart window over the control
#                                range is under its limit
#   physics       every rendering check, at TWO rasters: 320x180, which is
#                 what CI renders at, and 1280x720. Each is measured out of
#                 the picture:
#                   --echo       a delay of a line + k samples ghosts one line
#                                down and k across; padding moves it as the
#                                stride predicts
#                   --feedback   the nth echo at n ( k, 1 ) with amplitude m g^n
#                   --interleave a delay = 1 (mod 3) puts R's echo in G, G's
#                                in B, B's in the next pixel's R
#                   --flanger    the ghost's offset on every line is the LFO's
#                                value at that stream time
#                   --allpass    the phaser keeps a noise line's energy; the
#                                windowed GPU form matches the serial cascade
#                   --wrap       8-bit wraps past full scale, 16-bit wraps,
#                                float clips
#                   --pitch      a bar pattern's period is stretched by 1 / r
#                   --negative   every one of those FAILS on a perturbed effect
...
step "shaders"
if out=$(tools/check-shaders.sh "$DBTEST" 2>&1); then
	pass "$( printf '%s\n' "$out" | tail -1 | sed 's/^ *//' )"
else
...
step "offline (no GL)"
for check in laws names window; do
	if out=$("$DBTEST" --$check 2>&1); then
		summary=$( printf '%s\n' "$out" | grep -v '^$' | grep -v 'checks,' | grep -v '^   ' | tail -1 )
		pass "dbtest --$check: $summary"
	else
		fail "dbtest --$check"
		printf '%s\n' "$out" | sed 's/^/      /'
	fi
...
for size in 320x180 1280x720; do
	...
	for check in echo feedback interleave flanger allpass wrap pitch negative; do
		if out=$("$DBTEST" --$check --size $size 2>&1); then
			pass "dbtest --$check: $( printf '%s\n' "$out" | grep -v '^$' | grep -v 'checks,' | tail -1 )"
		else
			fail "dbtest --$check at $size"
			printf '%s\n' "$out" | sed 's/^/      /'
		fi
	done
done
...
got=$( "$DBTEST" --pipe --size 64x36 < "$raw" 2>/dev/null | wc -c | tr -d ' ' )
status=${PIPESTATUS[0]}
...
# pipeline whatever dbtest did, and the refusal would pass for the wrong reason.
printf '0 No Such Control 0.5\n' > "$cues"
"$DBTEST" --pipe --size 64x36 --script "$cues" < "$raw" >/dev/null 2>&1
status=$?
if [ "$status" -eq 2 ]; then
...
got=$( "$DBTEST" --pipe --size 64x36 --fail-render-at 1 < "$raw" 2>/dev/null | wc -c | tr -d ' ' )
status=${PIPESTATUS[0]}
...
"$DBTEST" --pipe --size 64x36 < "$many" 2>/dev/null | head -c 1 >/dev/null
status=${PIPESTATUS[0]}
if [ "$status" -eq 1 ]; then
...
step "sweep"
if out=$(python3 tools/sweep.py --binary "$DBTEST" 2>/dev/null); then
	pass "$( printf '%s\n' "$out" | tail -1 )"
else
...
"$DBTEST" --bench --frames 60 2>&1 | sed -n '3,6p' | sed 's/^/   /'

BUNDLE="$BUILD/Databend.bundle"
BIN="$BUNDLE/Contents/MacOS/Databend"
...
if [ "$(uname)" = "Darwin" ] && [ -d "$BUNDLE" ]; then
	...
	if [ "$ident" = "com.stoatworks.ffgl.databend" ]; then
	...
	if codesign --force --sign - --timestamp=none "$tmp/Databend.bundle" >/dev/null 2>&1; then
	...
	if [ -x "$OXBOW" ]; then
		...
		for want in "name:        SW Databend" "id:          DB01" "type:        effect"; do
			case "$probe" in
