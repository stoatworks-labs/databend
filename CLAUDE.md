...
- **Never loaded into Resolume on macOS.** Everything numeric is measured offline on
  macOS, plus an `oxbow` load. Footage through `--pipe` only (the defaults were judged
  on Resolume's bundled demo clips that way, and the release video was rendered so).
- Windows: built by CI and gated in Arena on win-lab (software rendering); see AGENTS.md.
...
- `StoatworksAbout.h` and `ATTRIBUTIONS.md` are GENERATED by the backend's syncs
  (`sync-about.py`, `sync-attributions.py`); edit the source tables there, not here.
